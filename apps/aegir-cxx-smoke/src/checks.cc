...
int run()
{
    /* Raw malloc/free first: this is musl's own path, before any container. */
    ...
    /* std::unordered_map is the check after this one, and it is deliberately
     * not here yet: its first insertion needs a stack larger than the 8 KiB
     * floor, and asking for one runs into the allocator's fixed
     * untyped-record table (specs/userland.md's spawn-parameter note). The
     * containers below prove the allocation path in the meantime. */
    ...
    /* Free and reallocate the same size: mallocng's groups should hand the
     * chunk back, which is what makes this heap freeing rather than a bump. */
    {
